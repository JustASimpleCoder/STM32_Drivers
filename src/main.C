...
void MPU9250_ReadAccelGyro(float *accel, float *gyro);
...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  uart_buf[0] = REG_TEMP;
	  ret = HAL_I2C_Master_Transmit(&hi2c1, MPU9250_ADDR, uart_buf, 1, HAL_MAX_DELAY);

	  if(ret != HAL_OK){
		  strcpy((char*)uart_buf, "Error Tx\r\n");
	  }else{
		  ret = HAL_I2C_Master_Receive(&hi2c1, MPU9250_ADDR, uart_buf, 2, HAL_MAX_DELAY);
		  if(ret != HAL_OK){
			  strcpy((char*)uart_buf, "Error Rx\r\n");
		  }else{
...
void MPU9250_ReadAccelGyro(float *accel, float *gyro)
{
    ...
    accel[0] = (int16_t)((imu_buf[0] << 8) | imu_buf[1]) / 16384.0;
    accel[1] = (int16_t)((imu_buf[2] << 8) | imu_buf[3]) / 16384.0;
    accel[2] = (int16_t)((imu_buf[4] << 8) | imu_buf[5]) / 16384.0;

    // Convert gyroscope raw data to degrees/second
    gyro[0] = (int16_t)((imu_buf[8] << 8) | imu_buf[9]) / 131.0;
    gyro[1] = (int16_t)((imu_buf[10] << 8) | imu_buf[11]) / 131.0;
    gyro[2] = (int16_t)((imu_buf[12] << 8) | imu_buf[13]) / 131.0;
}
